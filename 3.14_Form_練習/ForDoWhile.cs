    ...
    public partial class ForDoWhile : Form
    {
        ...
        private void btnFor_Click(object sender, EventArgs e)
        {
            ...
            if (isnum())
            {
                for (int i = A; i <= B; i += C)
                    sum += i;
                labResult.Text = A.ToString() + "  到  " + B.ToString() + "  相隔  " + (C - 1).ToString() + "\r\n加總為  " + sum;
            }
        }        
        ...
        private void btn99_Click(object sender, EventArgs e)
        {
            ...
            for(int i = 1; i <= 9; i++)
            {
                for(int j = 2; j <= 9; j++)
                {
                    string 積 = i * j < 10 ? "  " + i * j : (i * j).ToString();
                    s.Append(" " + j + "x " + i + "=" + 積 + " | ");
                }
                if (i != 9)
                {
